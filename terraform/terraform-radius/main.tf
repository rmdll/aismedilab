...
resource "vsphere_virtual_machine" "vm" {
 ...
 disk {
   label            = "${var.vm-name}-${count.index + 1}-disk-addon"
   thin_provisioned = true
   eagerly_scrub    = false
   size             = 3
  unit_number = 1
  ...
  clone {
    ...
    customize {
      ...
      network_interface {
        ipv4_address = "${var.vm-network-ipv4}.${var.vm-network-ipv4-vlanid}.${count.index + 1}"
        ipv4_netmask = var.vm-network-ipv4-netmask
	dns_server_list = var.vm-network-ipv4-dns
      }
      ipv4_gateway = "${var.vm-network-ipv4}.${var.vm-network-ipv4-vlanid}.${var.vm-network-ipv4-gateway}"
      timeout = 30
    }
  }
  connection {
  ...
  provisioner "remote-exec" {
        inline = [
                "sudo -u root bash -c \"pushd /etc/ansible/playbooks/scripts && ./init_config $(echo ${var.vm-name}-${count.index + 1} |tr [:upper:] [:lower:]) ${var.vm-network-ipv4}.${var.vm-network-ipv4-vlanid}.${count.index + 1} && popd\""
	]
  }
